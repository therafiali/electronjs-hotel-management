...
class PDFCreator {
  ...
  private async getInvoiceFromDatabase(invoiceId: string): Promise<any> {
    ...
    invoices.slice(0, 3).forEach((inv, index) => {
      console.log(`📋 Invoice ${index + 1}:`, {
        ...
        guestName: (inv.guestInfo as any)?.name || 'No name',
        total: inv.total
      });
    });
    ...
    console.log(`✅ Found invoice:`, {
      ...
      guestName: (invoice.guestInfo as any)?.name,
      total: invoice.total,
  ...
  private drawGuestInformation(doc: PDFKit.PDFDocument, invoiceData: InvoiceData) {
    ...
    if (invoiceData.roomInfo?.roomNumber) {
      ...
      doc
        ...
        .text(
          `Rs. ${(
            invoiceData.roomInfo.pricePerNight * (invoiceData.roomInfo.nights || 1)
          ).toFixed(2)}`,
          v,
          y + 72
        );
  ...
  private drawChargesTable(doc: PDFKit.PDFDocument, invoiceData: InvoiceData) {
    ...
    const row = (bg: string, texts: [string, string, string, string]) => {
